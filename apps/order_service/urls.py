...
urlpatterns = [
    ...
    url(r'order/service/set_invoice/(?P<pk>\w+)/', staff_member_required(SetInvoice.as_view()), name='set_invoice'),
    url(r'order/service/invoice/(?P<pk>\w+)/', staff_member_required(CheckInvoice.as_view()), name='order_invoice')
]
